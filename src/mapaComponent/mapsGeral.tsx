...
export default function MapaGeral() {
  ...
  const cardPorcentagem = larguraTela * 0.7
    ...
    <>
      <View className="flex-1 relative" onLayout={(event) => {
        ...
        <View style={{
          width:"93%",
          left: 15,
          right: 0, borderColor: theme.colors.text.disabled
        }} className=' 

        absolute z-10   top-[76%] rounded-[10rem] flex items-center justify-center border-[3px] ' >
          <View style={{
            backgroundColor: dark ? `${theme.colors.cards.zeBigode}50` : `${theme.colors.cards.zeBigode2}80`
          }} className='  w-full h-[75px] rounded-[10rem] flex flex-row items-center justify-center gap-3' >
            {pontos.map((ponto) => (
